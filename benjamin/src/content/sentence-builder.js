...
export function buildShiftLog({
  // The building name comes from the site profile in the config; the
  // fallback is a blank rather than a name so a build that forgets to set
  // one is obvious in the log instead of silently claiming a building.
  site = BLANK,
  shift = null,
  sections = [],
  state = {},
