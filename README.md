...
the building and the policies that apply at it.

**They ship inside the extension.** `MANUAL_URL` and `SITE_INFO_URL` at
the top of that build's `franklin-helper.js` are paths — `docs/manual.html`
and `docs/site-info.html` — resolved against the extension itself, so the
pages open with no account, no network and nothing hosted elsewhere. Set
one to a full `https://` URL to point at a hosted copy instead, or to an
